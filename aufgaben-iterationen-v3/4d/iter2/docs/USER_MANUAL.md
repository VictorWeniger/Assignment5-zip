...
2. Set environment variables if needed:
   ...
   - `MPE_NLP_ENGINE` (`local`, `duui`, `auto`; default: `duui`)
...
- Use `.credentials/mpe.properties` as the active local credentials file.
...
- `duui`: run DUUI Java composer pipeline (Assignment 4 style).
- `auto`: behaves like `duui` for assignment compliance and fails fast if DUUI targets are missing.
